...
export function malAnime2DynamodbAnime(malAnime: any): Anime {
  const {
    id,
    title,
    main_picture,
...
export function newAnimeFromMal(oldAnime: Anime, newAnime: Anime) {
  const propsToUpdate: (keyof Anime)[] = [
    'yearSeason',
    'picture',
    'type',
    'status',
...
export function newAnimeFromAcg(oldAnime: Anime, newAnime: Anime) {
  const propsToUpdate: (keyof Anime)[] = ['title', 'summary']
  const modifiedItem: any = {}
  for (const prop of propsToUpdate) {
